def aStarAlgo(start_node, stop_node):
    ...
    while len(open_set) > 0:
        ...
        # *** CORRECTION 2: Simplified 'if' condition ***
        # The original check would cause a KeyError.
        if n == stop_node:
            break
        else:
            # This loop is now safe thanks to the fix in get_neighbors
        ...
        # This check is needed in case n was not found (e.g., open_set was empty)
        # Although the while loop should prevent this, it's safer
        if n is None:
    ...
    if n != stop_node: # Check if we exited the loop without finding the stop node
        print('Path does not exist!')
        return None
...
def get_neighbors(v):
    if v in Graph_nodes:
        ...
        # *** CORRECTION 1: Return an empty list instead of None ***
        return []
...
def heuristic(n):
    ...
    # Use .get() for safety, returning a high value if node not in heuristic
...
# *** CORRECTION 3: Removed the extra text from the function call ***
aStarAlgo('A', 'J')